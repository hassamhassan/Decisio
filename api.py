    ...
    if incident_existed_in_db:
        async with get_session() as session:
            await crud.add_qa_record(
                session,
                incident_id=incident_id,
                question=(current_q or {}).get("question", ""),
                answer=answer,
                category=(current_q or {}).get("category", "general"),
...
async def submit_outcome(incident_id: str, req: OutcomeRequest, user: TokenData = Depends(require_auth)):
    ...
    if status == "AWAITING_VERIFICATION":
        # Don't close yet — need verification (§13)
        pass
...
async def super_admin_list_admins(super_admin: TokenData = Depends(require_super_admin)):
    ...
    async with get_session() as session:
        result = await session.execute(
            sa_select(User, Company.name.label("company_name"), Company.slug.label("company_slug"))
...
async def super_admin_update_company(
    ...
    """Update a company name and slug (super_admin only)."""
    async with get_session() as session:
        ...
        if existing.first():
            raise HTTPException(409, "Company name or slug already in use by another company")
        company.name = req.name.strip()
        company.slug = req.slug.strip()
        await session.flush()
        return {
